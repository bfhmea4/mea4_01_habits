...
const ProfileSettingsForm = () => {
  ...
  const handleVerifyEmail = async () => {
    await client.users
      .requestVerification(userObj.email)
      .then(() => {
  ...
  const handleRequestPasswordReset = async () => {
    await client.users
      .requestPasswordReset(userObj.email)
      .then(() => {
        Toast('Email sent', ToastType.success)
      })
      .catch((err: ClientResponseError) => {
    ...
    <div className="max-w-2xl mx-auto mt-10">
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="relative">
          ...
          {!userObj?.verified ? (
            <div className="absolute flex flex-row gap-1 justify-center items-center text-red-400 right-2 top-2">
              <XCircleIcon className="w-4 h-4 inline" />
              <span className="text-xs ">Email not verified</span>
            </div>
          ) : (
            <div className="absolute flex flex-row gap-1 justify-center items-center text-green-400 right-2 top-2">
          ...
          {!userObj?.verified && (
            <div className="grid w-full sm:grid-cols-2 gap-4 justify-center items-center">
              <div />
              <StyledButton
                name="Verify email"
                type={StyledButtonType.Secondary}
