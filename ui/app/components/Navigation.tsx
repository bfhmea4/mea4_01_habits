      ...
      <div className="flex flex-col space-y-8 items-center">
        {navigation.map(item => (
          <div key={item.name} onClick={() => router.push(item.href)}>
            <Tooltip content={item.name} placement="right" arrow={false}>
              <div
        ...
        <Menu as="div" className="bottom-4 absolute">
          ...
          >
            <Menu.Items className="absolute bottom-0 z-50 mb-10 w-48 origin-bottom-left rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
              <Menu.Item>
                {({ active }) => (
                  <div className="px-4 py-2 text-sm border-b-2 shadow-sm">
                    <div className="">{userObj?.profile?.name}</div>
                    <div className="truncatefont-GilroyMedium">{userObj?.email}</div>
                  </div>
                )}
              </Menu.Item>
              <Menu.Item>
